import type { Project } from '../types'
import styles from './ProjectsPage.module.css'

const GRADIENTS = [
  'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)',
  'linear-gradient(135deg, #1a0533 0%, #2d0b4e 50%, #4a1576 100%)',
  'linear-gradient(135deg, #0d1f0d 0%, #1a3a1a 50%, #0f4010 100%)',
  'linear-gradient(135deg, #1f1a0d 0%, #3a2e0d 50%, #4a3a0a 100%)',
]

interface Props {
  projects: Project[]
  loading: boolean
  onSelect: (id: string) => void
}

export function ProjectsPage({ projects, loading, onSelect }: Props) {
  if (loading) {
    return (
      <div className={styles.loading}>
        <div className={styles.spinner} />
      </div>
    )
  }

  return (
    <div className={styles.page}>
      <div className={styles.header}>
        <h1 className={styles.title}>Projets</h1>
        <p className={styles.subtitle}>{projects.length} projet{projects.length !== 1 ? 's' : ''}</p>
      </div>

      <div className={styles.grid}>
        {projects.map((project, i) => (
          <div key={project.id} className={styles.card} onClick={() => onSelect(project.id)}>
            <div className={styles.preview}>
              {project.image_url ? (
                <img src={project.image_url} alt={project.title} className={styles.previewImg} />
              ) : (
                <div
                  className={styles.previewPlaceholder}
                  style={{ background: GRADIENTS[i % GRADIENTS.length] }}
                >
                  <div className={styles.mockLines}>
                    <div className={`${styles.bar} ${styles.accent}`} />
                    <div className={styles.bar} />
                    <div className={`${styles.bar} ${styles.short}`} />
                  </div>
                </div>
              )}
            </div>

            <div className={styles.meta}>
              <div className={styles.info}>
                <h3 className={styles.cardTitle}>{project.title}</h3>
                <p className={styles.desc}>{project.description}</p>
                <div className={styles.techs}>
                  {project.technologies.slice(0, 4).map((t) => (
                    <span key={t} className={styles.tech}>{t}</span>
                  ))}
                  {project.technologies.length > 4 && (
                    <span className={styles.tech}>+{project.technologies.length - 4}</span>
                  )}
                </div>
              </div>

              <div className={styles.links} onClick={(e) => e.stopPropagation()}>
                {project.live_url && (
                  <a href={project.live_url} target="_blank" rel="noopener noreferrer" className={styles.linkBtn} title="Voir le projet">
                    <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2">
                      <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" />
                      <polyline points="15 3 21 3 21 9" />
                      <line x1="10" y1="14" x2="21" y2="3" />
                    </svg>
                  </a>
                )}
                <a href={project.github_url} target="_blank" rel="noopener noreferrer" className={styles.linkBtn} title="GitHub">
                  <svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M9 19c-5 1.5-5-2.5-7-3m14 6v-3.87a3.37 3.37 0 0 0-.94-2.61c3.14-.35 6.44-1.54 6.44-7A5.44 5.44 0 0 0 20 4.77 5.07 5.07 0 0 0 19.91 1S18.73.65 16 2.48a13.38 13.38 0 0 0-7 0C6.27.65 5.09 1 5.09 1A5.07 5.07 0 0 0 5 4.77a5.44 5.44 0 0 0-1.5 3.78c0 5.42 3.3 6.61 6.44 7A3.37 3.37 0 0 0 9 18.13V22" />
                  </svg>
                </a>
              </div>
            </div>
          </div>
        ))}
      </div>
    </div>
  )
}
